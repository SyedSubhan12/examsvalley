// EXTRACTED FROM: client/src/api/admin.ts
// CONVERTED TO:   lib/api/admin.ts
// BUCKET:         A_reuse
// WEB LIBRARIES REPLACED: none
// LOGIC CHANGES: uploadFileAsset fetch URL updated to use EXPO_PUBLIC_API_URL; import path kept as @/lib/queryClient
...
export async function uploadFileAsset(formData: FormData): Promise<FileAsset> {
  const res = await fetch(`${process.env.EXPO_PUBLIC_API_URL}/api/curriculum/files/upload`, {
    method: "POST",
    body: formData,
    credentials: "include",
  });
